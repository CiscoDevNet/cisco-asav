...
def get_user_input_configure_asav():
    ...
    try:
        user_input['ConfigureASAvTopic'] = os.environ['CLS_MANAGER_TOPIC']
        ...
        if user_input['kms_enc'] is None:
            user_input['AutoScaleUserPassword'] = os.environ['ASAv_PASSWORD']
        else:
            user_input['AutoScaleUserPassword'] = get_decrypted_key(os.environ['ASAv_PASSWORD'])
        user_input['GWLB_ARN'] = os.environ['GWLB_ARN']
...
def sns_msg_body_configure_asav_topic(message, to_function, category, instance_id, counter='-1'):
    ...
    """
    if counter == '-1':
        if to_function == 'cluster_configure':
            counter = 3
        elif to_function == 'cluster_delete':
            counter = 3
        elif to_function == 'cluster_ready':
            counter = 3
        elif to_function == 'cluster_license':
            counter = 5
