...
def lifecycle_deregister_smart_license(instance_id):
    ...
    message_subject = 'EVENT: ' + user_input['AutoScaleGrpName'] + ' ' + 'instance deregister' + ' ' + \
                      instance_id
    msg_body = utl.sns_msg_body_configure_asav_topic('VM De-register smart license', 'vm_delete', 'SECOND',
                                                     instance_id)
    sns.publish_to_topic(user_input['CONFIGURE_ASAV_TOPIC_ARN'], message_subject, msg_body)
