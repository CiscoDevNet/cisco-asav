...
Name:       oci_asav_autoscale_teardown.py
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="*** Script to destroy full end-to-end OCI-ASAv Autoscale Solution (Stack-1, Oracle-Function and Stack-2) ***\nPlease provide the required input in destroy_solution_parameters.txt", formatter_class=RawTextHelpFormatter)
    ...
    try:
        ...
        if stack2_destroy_response.lifecycle_state != "SUCCEEDED":
            print("\nERROR IN DESTROYING RESOURCES OF STACK-2")
            exit(1)
        else:
        ...
        if stack1_destroy_response.lifecycle_state != "SUCCEEDED":
            print("\nERROR IN DESTROYING RESOURCES OF STACK-1")
            exit(1)
        else:
        ...
        print("\nUNEXPECTED ERROR OCCURRED, PLEASE DESTROY AND DELETE THE STACK MANUALLY", end="\n")
        print(e)
        exit(1)
