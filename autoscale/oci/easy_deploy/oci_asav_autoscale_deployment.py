...
Name:       oci_asav_autoscale_deployment.py
...
from oci_asav_autoscale_teardown import Cleanup
...
class Stack_Deploy:
    def __init__(self):
        ...
        self.resourceManagerClient = oci.resource_manager.ResourceManagerClient(config={}, signer=self.auth)
    ...
    def parameter_validation(self):
        try:
            ...
            scaling_thresholds = str(parameters["scaling_thresholds"])
            min_threshold, max_threshold = scaling_thresholds.split(',')
            if not min_threshold.isdigit() or not max_threshold.isdigit():
                raise Exception("scaling_thresholds is not a number, Please verify again")
            if int(min_threshold) > int(max_threshold):
                raise Exception("scaling_thresholds is incorrect, min cant be greater than or equals to max, Please verify again")
            if int(min_threshold) > 100 or int(min_threshold) <0:
                raise Exception("scaling_thresholds min value should be between 0 and 100, Please verify again")
            if int(max_threshold) > 100 or int(max_threshold) <0:
                raise Exception("scaling_thresholds max value should be between 0 and 100, Please verify again")
    ...
    def create_logs(self, jobId, stackName):
        try:
            ...
            log_file_name = stackName + "_Error.log"
...
if __name__ == "__main__":
    ...
    ###############################################################################################
        try:
            ...
            if resource1_create_response == None:
                ...
                print("PLEASE MANUALLY DESTROY AND DELETE STACK AND ALL THE RESOURCES")
                exit(1)
    ...
    ###############################################################################################
        try:
            ...
            if resource2_create_response == None:
                print("EXCEPTION OCCURRED IN CREATING RESOURCES OF {}".format(stack2_Name))
    ...
    ###############################################################################################
        ...
        print("\nCOMPLETE END TO END OCI ASAv AUTOSCALE SOLUTION HAS BEEN DEPLOYED\nVERIFY MANUALLY TOO BEFORE ACTUAL USAGE\nTHANK YOU !!!")
        exit(0)
    except Exception as e:
        print(e)
